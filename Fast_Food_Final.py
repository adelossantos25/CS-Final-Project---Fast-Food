...
Description:
This program allows users to explore a dataset of fast food restaurants in the USA through an interactive web interface.
The program uses Streamlit to create an easy-to-use platform where users can filter and visualize fast food restaurant data
based on categories, provinces, cities, and more.
The program includes interactive features such as a map view, bar chart, pie chart, and a data table for in-depth exploration.
"""
...
if df is not None:  # Check if the data was loaded successfully
    ...
    df['categories'] = df['categories'].str.replace(r"[,/]", " and ",regex=True)  # Replace commas and slashes with " and "
    ...
    # Function to map raw categories to broader categories
...
# 1. Select Categories
selected_category = st.sidebar.multiselect("Select Categories",options=list(category_map.keys()))  # Multi-select for categories
...
if df is not None:
    ...
    if selected_category:
        filtered_restaurant_df = filtered_restaurant_df[  # Filter based on selected categories
...
if df is not None:
    ...
    if selected_category:
        filtered_df = filtered_df[  # Filter based on selected categories
    ...
    if not filtered_df.empty:
        ...
        if unique_provinces == 1:
            ...
            ax.pie(province_counts, labels=province_counts.index, autopct='%1.1f%%', startangle=90,
                   colors=plt.cm.Paired.colors)
            ax.set_title("Province Distribution")
            st.pyplot(fig)

# Filtered Data
with tabs[3]:
    if not filtered_df.empty:
        st.markdown("### Filtered Data")  # Subtitle for the data table
        st.dataframe(filtered_df)  # Display the filtered data table
